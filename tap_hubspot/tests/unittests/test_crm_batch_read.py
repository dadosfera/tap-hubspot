...
class TestCrmBatchRead(unittest.TestCase):
    ...
    def test_ids_are_split_at_the_supported_batch_limit(self, list_request, batch_request):
        ...
        self.assertEqual([str(index) for index in range(205)], [row['id'] for row in rows])
    ...
    @patch("tap_hubspot.post_search_endpoint")
    @patch("tap_hubspot.request")
    def test_missing_batch_id_fails_instead_of_emitting_partial_records(
        self, list_request, batch_request
    ):
        """Every ID returned by the list endpoint must be present in Batch Read."""
        list_request.return_value = MockResponse(
            {"results": [crm_row("1"), crm_row("2")]}
        )
        batch_request.return_value = MockResponse(
            {"results": [crm_row("1", properties={"subject": "value"})]}
        )

        with self.assertRaisesRegex(
            RuntimeError, "Batch read response did not include requested record id 2"
        ):
            list(
                tap_hubspot.get_v3_records_with_batch_read(
                    "list-url", {}, "batch-url", ["subject"]
    ...
    @patch("tap_hubspot.get_start", return_value="2024-01-01T00:00:00Z")
    @patch("tap_hubspot.get_v3_records_with_batch_read")
    def test_http_429_batch_failure_does_not_advance_bookmark(
        self, batch_records, _get_start, _load_schema, _write_schema,
        write_record, write_bookmark
    ):
        """An exhausted Batch Read rate limit cannot commit a final bookmark."""
        response = requests.Response()
        response.status_code = 429
        error = requests.exceptions.HTTPError(response=response)

        def first_batch_then_rate_limit():
            yield crm_row("1")
            raise error

        batch_records.return_value = first_batch_then_rate_limit()
        context = MockContext("contacts", ["firstname"])
    ...
    def test_http_failures_after_first_page_preserve_bookmarks(self):
        """Contacts and Tickets keep their bookmark after a later 429 or 5xx."""
        for stream_id in ("contacts", "tickets"):
            for status_code in (429, 500, 503):
                with self.subTest(stream=stream_id, status=status_code):
                    response = requests.Response()
                    response.status_code = status_code
                    error = requests.exceptions.HTTPError(response=response)
                    self._assert_later_batch_failure_preserves_bookmark(stream_id, error)

    def test_missing_id_after_first_page_preserves_bookmarks(self):
        """An incomplete second batch cannot advance either stream's bookmark."""
        for stream_id in ("contacts", "tickets"):
            with self.subTest(stream=stream_id):
                self._assert_later_batch_failure_preserves_bookmark(
                    stream_id, MockResponse({"results": []})
                )

    def _assert_later_batch_failure_preserves_bookmark(self, stream_id, failure):
        """Exercise real list/batch iteration and sync with synthetic responses."""
        from copy import deepcopy  # pylint: disable=import-outside-toplevel

        state = {
            "currently_syncing": stream_id,
            "bookmarks": {stream_id: {"updatedAt": "2024-01-01T00:00:00Z"}},
        }
        original_state = deepcopy(state)
        pages = [
            MockResponse({"results": [crm_row("1")],
        ...
        with patch("tap_hubspot.request", side_effect=pages), \
             ...
             patch("tap_hubspot.load_schema", return_value={"type": "object", "properties": {}}), \
             patch("tap_hubspot.singer.write_schema"), \
             patch("tap_hubspot.singer.write_record") as write_record, \
             patch("tap_hubspot.singer.write_bookmark") as write_bookmark, \
             patch("tap_hubspot.singer.write_state") as write_state:
            expected_error = type(failure) if isinstance(failure, Exception) else RuntimeError
            with self.assertRaises(expected_error):
                getattr(tap_hubspot, "sync_" + stream_id)(state, context)
            write_record.assert_called_once()
            write_bookmark.assert_not_called()
